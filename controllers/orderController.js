...
const resolveItems = async (rawItems) => {
  ...
  const resolved = await Promise.all(
    rawItems.map(async (it, idx) => {
      ...
      // ✅ 🔥 PACKAGING LOGIC ADD
      const weight = product.weight || { value: 0, unit: "pcs" };
...
// Controller level pe use hota hai createOrder aur updateOrderItems mein
...
exports.updateOrderItems = async (req, res) => {
  try {
    ...
    // GST summary updated items se banao
    const gstSummary = buildGstSummary(resolved);
